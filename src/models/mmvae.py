...
class MMVAE(nn.Module):
    ...
    def forward(self, x, K=1):
        ...
        Returns
        -------
            qz_xs: List[torch.distributions]
            px_zs: List[List[torch.distributions]]
            zss: List[torch.Tensor]

        qz_xs:
            qz_xs[0] -> q_theta_1 (z|x_1), encoder output for x_1 (mnist) 
                qz_xs[0].rsample().shape = [1, bs, 20]
            qz_xs[1] -> q_theta_2 (z|x_2), encoder output for x_2 (svhn)
                qz_xs[1].rsample().shape = [1, bs, 20]

        zss: 
            zss[0]: samples from q_theta_1 (z|x_1), [1, bs, 20]
            zss[1]: samples from q_theta_2 (z|x_2), [1, bs, 20]

        px_zs:
            px_zs[0][0] -> p_theta_1 (x_1|zss[0]), rsample_shape: [1, bs, 1, 28, 28]
            px_zs[1][1] -> p_theta_2 (x_2|zss[1]), rsample_shape: [1, bs, 3, 32, 32]
            px_zs[0][1] -> p_theta_2 (x_2|zss[0]), rsample_shape: [1, bs, 3, 32, 32]
            px_zs[1][0] -> p_theta_1 (x_1|zss[1]), rsample_shape: [1, bs, 1, 28, 28]
        '''
        qz_xs, zss = [], []
