...
def custom_make_mnist_svhn_idx(max_d: int, dm: int):
    # max_d: maximum number of datapoints per class
    # dm: data multiplier: random permutations to match
    ...
    torch.save(idx1, f'../data/train-ms-mnist-idx_max_d_{max_d}_dm_{dm}.pt')
    torch.save(idx2, f'../data/train-ms-svhn-idx_max_d_{max_d}_dm_{dm}.pt')

    mnist_l, mnist_li = test_mnist.targets.sort()
    svhn_l, svhn_li = test_svhn.labels.sort()
    
    ####################################### ^?
    idx1, idx2 = rand_match_on_idx(mnist_l, mnist_li, svhn_l, svhn_li, max_d=int(max_d//5), dm=dm)
    print('len test idx:', len(idx1), len(idx2))
    torch.save(idx1, f'../data/test-ms-mnist-idx_max_d_{max_d}_dm_{dm}.pt')
    torch.save(idx2, f'../data/test-ms-svhn-idx_max_d_{max_d}_dm_{dm}.pt')
